...
<body>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr data-order-id="{{ $order->id }}">
                        ...
                        <td class="timestamp">{{ $order->created_at->format('d/m/Y H:i') }}</td>
                        <td>
    ...
    <script>
        ...
        function updateStatus(orderId, newStatus) {
            fetch('{{ route("admin.updateStatus", ":id") }}'.replace(':id', orderId), {
                method: 'POST',
                headers: {
        ...
        function deleteOrder(orderId) {
            if (confirm('⚠️ Apakah Anda yakin ingin menghapus pesanan ini?')) {
                fetch('{{ route("admin.deleteOrder", ":id") }}'.replace(':id', orderId), {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        async function fetchDashboardData() {
            try {
                ...
                const response = await fetch('{{ route("admin.dashboardData") }}', {
    ...
    <script>
        ...
        try {
            if ('{{ env('PUSHER_APP_KEY') }}' && '{{ env('PUSHER_APP_KEY') }}' !== '') {
