...
<body>
    <div class="container">
        ...
        @if($orders->count() > 0)
            <div class="orders-container">
                ...
                <div class="order-card" data-status="{{ $order->status }}" onclick="viewOrder({{ $order->id }})">
                    <div class="order-header">
                        <div style="flex: 1;">
                            <div class="order-number">#{{ $order->order_number }}</div>
                            <div class="order-date">{{ $order->ordered_at ? $order->ordered_at->format('d M Y · H:i') : $order->created_at->format('d M Y · H:i') }}</div>
                        </div>
                    ...
                    <div class="order-details">
                        ...
                        <div class="detail-item">
                            ...
                            <div class="detail-value">{{ $order->items->count() }} item</div>
                        </div>
                    </div>
                    ...
                    <div class="order-footer">
                        ...
                        <a href="{{ route('order.track', $order->order_number) }}" class="action-button">Lihat Detil →</a>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="pagination">
                ...
                @else
                    <a href="{{ $orders->previousPageUrl() }}">← Sebelumnya</a>
                @endif
                ...
                @if($orders->hasMorePages())
                    <a href="{{ $orders->nextPageUrl() }}">Selanjutnya →</a>
                @else
        ...
        @else
            <div class="empty-state">
                ...
                <a href="{{ route('menu.index') }}" class="back-button">← Kembali ke Menu</a>
            </div>
        @endif
    </div>
    ...
    <script>
        ...
        window.Echo = new Echo({
            ...
            wsPort: {{ env('PUSHER_APP_PORT', 6001) }},
            wssPort: {{ env('PUSHER_APP_PORT', 6001) }},
            disableStats: true,
        });
